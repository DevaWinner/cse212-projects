...
public class TakingTurnsQueueTests
{
    ...
    // Defect(s) Found: The first person dequeued is Sue instead of Bob, which means the queue isn't handling turns correctly.
    ...
    // Defect(s) Found: After adding George, Sue is dequeued instead of Tim, so George isn't being added properly.
    ...
    // Run 10 times.
    // Expected Result: Bob, Tim, Sue, Bob, Tim, Sue, Tim, Sue, Tim, Tim
    // Defect(s) Found: The first person dequeued is Sue instead of Bob, so infinite turns is not working right.
    ...
    // Defect(s) Found: The first person dequeued is Sue instead of Tim, so negative turns is not handled as infinite.
